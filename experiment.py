...
class Experiment:
    ...
    def read_adjacent_matrix_from_file(self, filepath):
    ...
    def plot(self):
        ...
        data = pd.DataFrame({
            'group_A': self.susceptible_people_history,
            'group_B': self.infected_people_history,
            'group_C': self.immune_people_history
        }, index=range(1, len(self.susceptible_people_history) + 1))

        # We need to transform the data from raw data to percentage (fraction)
        data_perc = data.divide(data.sum(axis=1), axis=0)

        # Make the plot
        plt.stackplot(range(1, len(self.susceptible_people_history) + 1), data_perc["group_A"], data_perc["group_B"],
                      data_perc["group_C"],
